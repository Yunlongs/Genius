...
def retrieveFeatures(n, base_dir, filename):  # 究竟获得什么样的feature还未知
	'''
	n:第几个文件
	base_dir : 输入训练语料目录
...
def queryBytwo(base_input_dir, filename1, filename2, n):
	'''
	base_input_dir: 输入训练语料的目录
	filename1: target function
	filename2: query function
	n:
	'''
	threthold = 50
	db_instance = db()
	funcs = loadFuncs(base_input_dir)
	db_instance.loadHashmap(feature_size=n, result_n=1000)
	...
	ranks = []
	for threthold in range(1, 210, 10):
		hit = []
		i = 0
		for name in queries.keys():
			...
			if name not in featrues:
				continue
			query = queries[name]
			x = db_instance.engine.neighbours(query) #  x = (vector, data, distance)
			try:
				rank = [v for v in range(len(x)) if name in x[v][1]][0]
...
def queryAll(base_dir, firmware_name, filename1, n):
	...
	for name in queries:
		...
		#pdb.set_trace()
		try:
			rank = [v for v in range(len(x)) if name in x[v][1]]
			if len(rank) > 1:
				pdb.set_trace()
				print ("stop")
			if rank[0] <= threthold:
